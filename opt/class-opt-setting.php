 ...
 * @access public
 */
if(!class_exists('_Proton__Opt_Setting')):

class _Proton__Opt_Setting {
	...
	public function __construct( $manager, $name, $args = array() ) {
		...
		if ( $this->sanitize_callback )
			add_filter( "proton_opt_{$this->manager->name}_sanitize_{$this->name}", $this->sanitize_callback, 10, 2 );
	}
	...
	public function get_value() {
		...
		switch ($this->manager->type) {
			    ...
			    case 'meta':
			        ...
			        break;
			    case 'term':
			        $value = get_term_meta( $this->manager->post_id, $this->name, true );
			        break;
			    case 'user':
			        $value = get_user_meta( $this->manager->post_id, $this->name, true );
			        break;
			    default:
			        $value = get_post_meta( $this->manager->post_id, $this->name, true );
			}
		return ! $value && Proton::$opt->is_new_post ? $this->default : $value;
	}
	...
	public function get_field_name() {
		...
		return "proton_opt_{$this->manager->name}_setting_{$this->name}";
	}
	...
	public function sanitize( $value ) {
		...
		return apply_filters( "proton_opt_{$this->manager->name}_sanitize_{$this->name}", $value, $this );
	}
		...
		if ( ! $new_value && $old_value ){
			switch ($this->manager->type) {
			    ...
			    case 'meta':
			        ...
			        break;
			    case 'term':
			        delete_term_meta( $this->manager->post_id, $this->name );
			        break;
			    case 'user':
			        delete_user_meta( $this->manager->post_id, $this->name );
			        break;
			    default:
			        delete_post_meta( $this->manager->post_id, $this->name );
			}
		// If the new value doesn't match the old value, set it.
			...
			switch ($this->manager->type) {
			    ...
			    case 'meta':
			        ...
			        break;
			    case 'term':
			        update_term_meta( $this->manager->post_id, $this->name, $new_value );
			        break;
			    case 'user':
			        update_user_meta( $this->manager->post_id, $this->name, $new_value );
			        break;
			    default:
			        update_post_meta( $this->manager->post_id, $this->name, $new_value );
			}
