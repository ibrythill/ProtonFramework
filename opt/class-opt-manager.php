 ...
 * @access public
 */
if(!class_exists('_Proton__Opt_Manager')):

class _Proton__Opt_Manager {
	...
	public function register_section( $section, $args = array() ) {
		...
		if ( ! is_object( $section ) ) {
			...
			$type = isset( $args['type'] ) ? Proton::$opt->get_section_type( $args['type'] ) : Proton::$opt->get_section_type( 'default' );
	...
	public function register_control( $control, $args = array() ) {
		...
		if ( ! is_object( $control ) ) {
			...
			$type = isset( $args['type'] ) ? Proton::$opt->get_control_type( $args['type'] ) : Proton::$opt->get_control_type( 'default' );
	...
	public function register_setting( $setting, $args = array() ) {
		...
		if ( ! is_object( $setting ) ) {
			...
			$type = isset( $args['type'] ) ? Proton::$opt->get_setting_type( $args['type'] ) : Proton::$opt->get_setting_type( 'default' );
	...
	public function save( $post_id ) {
		...
		if ( ! isset( $_POST["proton_opt_{$this->name}"] ) || ! wp_verify_nonce( $_POST["proton_opt_{$this->name}"], "proton_opt_{$this->name}_nonce" ) )
			return;
	...
	public function print_template() { ?>
		...
		<script type="text/html" id="tmpl-proton-opt-manager-<?php echo esc_attr( $this->type ); ?>">
	...
	public function get_template() {
		proton_get_template('manager', $this->type );
	}
}
